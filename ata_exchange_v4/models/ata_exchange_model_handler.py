...
class AtaExchangeModelHandler(models.AbstractModel):
    ...
    def get_records(self,
        ...
        def need_rewrite_record(record_params: RecordHandlerParams):
            return record_params.write_record or \
                (inc_params:=record_params.incoming_params) and inc_params.method_id.parameter_always_rewrite

        Model = record_params.model
        records = self.search_records(record_params)
        if (not records and record_params.create_record) or (records and need_rewrite_record(record_params)):
            ...
            if vals:
                with self.env['ata.exchange.queue'].disable_add_temporarily(not record_params.add_to_queue):
                    ...
                    if isinstance(records, AtaExchangeModelHandlerMixin):
                        records.ata_exchange_after_write(record_params)
    ...
    def save_matching_data(self,
        ...
        if (search_params := record_params.search_params) and search_params.use_matching_data:
            key_object: str = record_params.data.get(search_params.key_matching_data, False)
            if (matching_data := self.env['ata.exchange.matching.data'].save_matching_data(
                record_params, key_object, records)):
                    record_params.response_data.matching_data.append(matching_data)
