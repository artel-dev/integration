...
def pre_init_hook(env):
    ...
    for view_name in views_to_delete:
        ...
        env.cr.execute("SELECT res_id FROM ir_model_data WHERE module = %s AND name = %s", (module_name, view_name))
        res = env.cr.fetchone()
        if not res:
        ...
        env.cr.execute("SELECT id FROM ir_ui_view WHERE id = %s", (view_id,))
